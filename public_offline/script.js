...
const DOMAIN = isSafariOniOS()
  ? `http://fund.sab.edu.vn`
  : `https://fund.sab.edu.vn`;
const ID_LENGTH = 8;
...
function updateCustomerInfo() {
  ...
  document.getElementById(
    "student-id-info"
  ).innerText = `Student ID: ${studentID}`;
...
function updateCheckoutRow(rowId, totalMoney) {
  document.getElementById(`${rowId}-total`).innerText =
...
function setQR() {
  const selectedKeys = [...order.events.keys()].filter(
    (key) => order.events.get(key) > 0
  );
  ...
  document
    ...
    .setAttribute(
      "src",
      `https://img.vietqr.io/image/${bankName}-${accountNum}-print.jpg?amount=${
        order.totalMoney
      }&addInfo=${msg.replaceAll(" ", "%20")}&accountName=${accountName}`
    );
}
